...
void main() {
    ...
    if (is_left_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_r];
    } else if (is_right_boundary) {
		p_buffer.p[cell] = p_buffer.p[cell_l];
    }
    if (is_top_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_d];
    } else if (is_bottom_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_u];
    }
    ...
    if (is_left_boundary && is_top_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_dr]; 
    } else if (is_left_boundary && is_bottom_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_ur]; 
    } else if (is_right_boundary && is_top_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_dl]; 
    } else if (is_right_boundary && is_bottom_boundary) {
        p_buffer.p[cell] = p_buffer.p[cell_ul];
    }
}